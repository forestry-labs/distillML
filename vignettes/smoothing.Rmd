...
$$
\widehat{m}_{h}(x)=\frac{\sum_{i=1}^{n} K_{h}\left(x-x_{i}\right) y_{i}}{\sum_{i=1}^{n} K_{h}\left(x-x_{i}\right)}
$$
...
$$
\hat{PDP}_{S, smooth}\left(x\right) =\frac{\sum_{i=1}^{n} K_{h}\left(x-x_{i}\right) \hat{PDP}_{S}\left(x_i\right) }{\sum_{i=1}^{n} K_{h}\left(x-x_{i}\right)}
$$
...
the parameters `smooth.binsize`, `smooth.npoints`, and `smooth.type` respectively. 
...
For the bandwidth, the default is the maximum difference in the grid points for 
