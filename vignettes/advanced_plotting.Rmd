...
## Advanced Feature: Smoothening

To get smoothened plots of PDP, ICE, or ALE curves for continuous features, we also 
have options to smoothen the curves using the `stats:ksmooth` package. This may be 
...
can set the bin size, number of points to interpolate, and the type of smoothening with 
the parameters `smooth.binsize`, `smooth.npoints`, and `smooth.type` respectively. These 
parameters correspond to 

```{r}
# Default smoothening options
plot(forest_interpreter,
     features = c("Petal.Length"),
     smooth = T)
...
the given feature. The type of smoothening done is by default gaussian, and 
